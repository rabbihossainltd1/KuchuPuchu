...
// ---- 8. owner identity: profile card, pure-Bangla rule, image fallback ----
{
  ...
  check(
    ...
    // 3D drop shadows are gone app-wide — the pin states the ABSENCE. (r72-16
    // briefly re-added a theme-adaptive lift; r73-16 removed it for good.)
    "no drop shadows anywhere in the app sources (owner order, r71-16 → r73-16) — the bubbles' lift was removed with the rest",
...
// ── r32: owner list ──────────────────────────────────────────────────────────
{
  ...
  // chip + blue send; captions ride the send path and render under bubbles.
  {
    ...
    check(
      "r34-16b: editor flow + captions — a grid tap ticks in place (strip + grid share the hd-aware toggle), the bar pencil opens the editor, the panel caption bar writes sel[0], the result carries the caption, MediaItem / the send path / forwards carry it (sealed from r64), captioned rows render it",
      (attach.match(/sel\.add\(item\.copy\(hd = hdOn\)\)/g) || []).length === 2 &&
        ...
        // r64 E2EE: the four payloads seal the caption (sealOut); the two
        // optimistic ECHO rows keep the plaintext the bubble reads.
        (chat.match(/\.put\("body", sealOut\(caption\)\)/g) || []).length === 4 &&

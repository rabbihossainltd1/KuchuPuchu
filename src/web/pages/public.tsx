...
export function LoginPage() {
  ...
  useEffect(() => {
    void api<{ google: boolean }>("/api/auth/providers")
    ...
    <AuthFrame>
      ...
      {google ? (
        ...
        <p className="meta" style={{ marginTop: 12 }}>
          Google sign-in stays off until OAuth credentials are configured.
        </p>
      )}
...
export function RegisterPage() {
  ...
  return (
    <AuthFrame>
      ...
      <p className="lede">Password needs 10+ characters, mixed case and a number.</p>
      {error ? <Notice tone="danger">{error}</Notice> : null}
      <form className="grid" style={{ marginTop: 18 }} onSubmit={onSubmit} noValidate>
        <label className="field">

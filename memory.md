...
- M5 decided: no code commit; end-release version bump is the fix (release discipline).
...
- E2 (typing/voice indicator 4px down) done, commit `PENDING-E2-SHA`, app-only (no worker change -> no deploy). TypingBubble + RecordingBubble rows: symmetric vertical 3dp -> top 7dp / bottom 3dp (layout-aware shift; offset() would overlap). test32 +1 E2 lock (both rows pinned, count==2). Gates: `npm run ci` exit 0, 41/41 (1648 assertions).
